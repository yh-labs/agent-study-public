...
> | v1.1 | 2026-06-29 | Service Worker/IndexedDB(오프라인), PDF Generator, Web Push → v1.1 추후 구현. 클라이언트 기술 스택 React+Vite → Next.js 16으로 수정 |
...
본 문서는 반려동물 건강 기록 웹앱(펫로그)의 인프라 구성, 컴포넌트 간 연계 방식, 배포 파이프라인, 보안 정책을 정의한다. 개발팀 및 운영팀이 시스템 전체 구조를 공유된 기준으로 이해하고 운영·확장할 수 있도록 하는 데 목적이 있다. (오프라인 지원 구조는 v1.1 추후 구현)

### 1.2 범위

| 구분             | 포함 내용                                                  |
| ---------------- | ---------------------------------------------------------- |
| 클라이언트       | Next.js 16 App Router + React 19 (Service Worker·IndexedDB는 v1.1 추후 구현) |
| 서버(Serverless) | Next.js Route Handlers (Vercel Serverless), Vercel Cron Jobs |
| 데이터 계층      | Supabase PostgreSQL (RLS), Supabase Storage                |
| 인증             | Supabase Auth (카카오·구글 OAuth 2.0)                     |
| 알림             | **v1.1 추후 구현** Web Push API (VAPID), FCM 호환         |
...
| 항목                     | 내용                                                                 |
| ------------------------ | -------------------------------------------------------------------- |
| 앱스토어 배포 없음       | 브라우저 기반 웹앱 단독 제공 (PWA 설치는 v1.1 추후 구현)           |
| 단일 배포 플랫폼         | 프론트엔드(Next.js)와 백엔드 API Route Handlers를 Vercel에서 통합 배포 |
| BaaS 의존                | 인증·DB·스토리지·RLS를 Supabase에 위임하여 자체 서버 운영 최소화  |
| 전 구간 HTTPS            | TLS 1.2 이상, HTTP 요청은 자동 리디렉션                              |
| Vercel Function 타임아웃 | 기본 10초 (PDF 생성은 v1.1 추후 구현 이후 maxDuration 60초 적용 검토) |
...
```mermaid
graph TB
    subgraph CLIENT["클라이언트 레이어 (Browser)"]
...
> **v1.1 추후 추가 예정**: Service Worker(오프라인), IndexedDB(pending_sync), PDF Generator(Puppeteer), Web Push Service(VAPID/FCM), Vercel Cron 리마인더 알림
...
| 오프라인 기록             | **[v1.1 추후 구현]** 브라우저 -> Service Worker -> IndexedDB -> Background Sync      |
| 알림 발송                 | **[v1.1 추후 구현]** Vercel Cron -> Serverless Function -> Web Push Service -> 기기  |
| PDF 생성                  | **[v1.1 추후 구현]** 브라우저 -> Serverless Function (maxDuration 60s) -> 다운로드   |

---

## 3. 컴포넌트별 상세 설명

### 3.1 프론트엔드 (Next.js App)

| 항목            | 내용                                                                       |
| --------------- | -------------------------------------------------------------------------- |
| 프레임워크      | Next.js 16 App Router + React 19                                           |
| 라우팅          | Next.js App Router 파일 기반 라우팅                                        |
| 차트            | Recharts (LineChart — 체중/음수량 추이) — 구현 시 설치 예정              |
| HTTP 클라이언트 | Fetch API (Supabase JS SDK 내장)                                           |
| 상태 관리       | React Context + useState (v1.0 범위)                                       |
| 빌드 출력       | Next.js 정적/SSR 혼합 — Vercel Edge CDN 서빙                              |
| PWA 설정        | **[v1.1 추후 구현]** `manifest.json` + Service Worker 등록               |

**[v1.1 추후 구현] PWA 지원 화면 범위**

| 화면                             | 오프라인 접근        | 캐싱 전략                    |
| -------------------------------- | -------------------- | ---------------------------- |
| 홈 (/)                           | 지원 예정            | Cache First (Service Worker) |
| 체중 기록 입력 (/record/weight)  | 지원 예정            | Cache First                  |
| 음수량 기록 입력 (/record/water) | 지원 예정            | Cache First                  |
| 리포트 목록·상세 (/reports/*)   | 미지원 (온라인 필요) | Network First                |
| 설정·마이페이지                 | 읽기만 지원 예정     | Cache First                  |

#### Service Worker 역할 [v1.1 추후 구현]
...
### 3.2 백엔드 (Next.js Route Handlers)

| 항목          | 내용                                                                                  |
| ------------- | ------------------------------------------------------------------------------------- |
| 런타임        | Node.js 20.x                                                                          |
| 프레임워크    | Next.js 16 App Router Route Handlers (`src/app/api/**`)                              |
| 진입점        | `src/app/api/` 폴더 내 각 `route.js` 파일                                           |
| 타임아웃      | 기본 10초 (PDF 생성은 v1.1 추후 구현 이후 60초 설정 검토)                            |
...
| `GET /api/pets/:id/reports/*`                   | 월간 리포트 조회 (PDF 생성은 v1.1 추후 구현)        |
| `GET/PATCH /api/notifications/*`               | 알림 설정 조회·변경 (구독 등록은 v1.1 추후 구현)   |
| `POST /api/cron/monthly-report`                 | Vercel Cron 전용 — 월간 리포트 생성               |
| ~~`POST /api/cron/daily-reminder`~~             | ~~Vercel Cron 전용 — 리마인더 발송~~ **[v1.1 추후 구현]** |
| `POST /api/cron/monthly-report`                 | Vercel Cron 전용 — 월간 리포트 생성                |
